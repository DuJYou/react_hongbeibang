...
class HotQuestons extends Component {
    state = {
        hotList: [],
        pageIndex:'0',
        switch: false
    ...
    async componentDidMount() {
        ...
        this.setState({
            hotList: result.data.content.data,
            switch: true
        })
          //上拉加载数据
    ...
    render(){
        return (
            ...
            <Answer>
                {this.state.hotList.map(value =><div key={value.contentQuestionId} onClick={this.handleDetail(value.recipeContentId)} className="Answer"> <div className="text">{value.title}</div>
...
export default withRouter(HotQuestons);